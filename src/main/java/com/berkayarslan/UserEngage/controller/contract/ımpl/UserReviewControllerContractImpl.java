...
public class UserReviewControllerContractImpl implements UserReviewControllerContract {
    ...
    public UserReviewDTO editComment(UserReviewUpdateRequest request) {
        reviewService.findByIdWithControl(request.id()).setComment(request.comment());
        return UserReviewMapper.INSTANCE.userReviewToUserReviewDTO(reviewService.findByIdWithControl(request.id()));
    }
}
